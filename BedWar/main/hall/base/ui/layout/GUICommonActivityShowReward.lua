...
local dialog ---@type GUICommonActivityShowReward

local GUICommonActivityShowReward = class("GUICommonActivityShowReward", IGUILayout) ---@class GUICommonActivityShowReward : IGUILayout
...
function GUICommonActivityShowReward:onLoad()
    ...
    self.llRewards = self:getChildWindow("CommonActivityShowReward-Rewards", GUIType.Layout)

    self.tvTitleText = self:getChildWindow("CommonActivityShowReward-Title", GUIType.StaticText)
    self.tvBottomTip = self:getChildWindow("CommonActivityShowReward-Bottom-Tip", GUIType.StaticText)
...
function GUICommonActivityShowReward:hide(force)
    if not force then
        return
    end
    if self:isShow() then
...
function GUICommonActivityShowReward:addRewardListItem(rewardList, allRewardCount, isOneReward)
    LuaTimer:cancel(self.showRewardKey)
    allRewardCount = allRewardCount or #rewardList
    local size = math.min(84, (self.llRewardBox:GetPixelSize().x - (allRewardCount - 1) * 22 - 20) / allRewardCount)
    ...
    local fun = function()
        ...
        if rewardInfo then
            if rewardInfo.type and rewardInfo.type == "extra" then
                ...
                rewardItem:setLuckyValue(rewardInfo)
                addItem(rewardItem)
            else
